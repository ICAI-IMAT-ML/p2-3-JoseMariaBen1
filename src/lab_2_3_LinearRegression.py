...
class LinearRegressor:
    ...
    def fit_simple(self, X, y):
        ...
        if np.ndim(X) > 1:
            X = X.reshape(1, -1)
        covarianza = sum((X-np.mean(X))*(y-np.mean(y)))/len(X)
        varianzax = (sum((X-np.mean(X))**2))/len(X)
        # TODO: Train linear regression model with only one coefficient
        self.coefficients = covarianza/varianzax
    ...
    def fit_multiple(self, X, y):
        ...
        w = (np.linalg.inv(X.T@X))@np.transpose(X)@y

        self.intercept = w[0]
        self.coefficients = np.array(w[1:])
    ...
    def predict(self, X):
        ...
        if np.ndim(X) == 1:
            ...
            predictions = self.intercept + self.coefficients@X.T
        return predictions
...
def sklearn_comparison(x, y, linreg):
    ...
    print("Scikit-Learn Coefficient:", sklearn_model.coef_)
